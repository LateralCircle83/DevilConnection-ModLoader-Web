...
Because profile transforms see the final mod-overlaid resource, they may constrain a mod that replaces the same path. Only genuine browser requirements belong there. A content-specific media patch may explicitly delegate a signature-unknown `mod` source to an existing Host runtime recovery; an unknown base-game source must still abort. Prefer adapter-level interception when a stable runtime contract exists; use source transformation only when the behavior cannot be corrected at a narrower boundary.
