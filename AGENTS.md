...
The Tyrano image-map touch guard is likewise a Host kernel responsibility because it closes the engine-level gap where Gecko delivers raw touch events to the image element instead of the `<area>` behind an `img[usemap]` (`<area>` has no layout box and Gecko deliberately does not retarget raw touch events, while compatibility mouse events are retargeted, which is why hover-only "selected" feedback appears without entry). Install the single delegated listener during adapter setup and confirm it again after mod hooks settle immediately before `TYRANO.init()`. It must drive only the game's existing tap path, stay inert under Chromium/WebKit area targeting and under PC native-click semantics, and must not change menu markup, rewrite `$.fn.click` / `$.event.tap`, or add global click/touch deduplication.
...
node tests\tyrano-image-map-touch.test.js
