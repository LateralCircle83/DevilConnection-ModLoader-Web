...
在线体验：https://lateralcircle83.github.io/DevilConnection-ModLoader-Web/

项目不包含游戏本体，也不会把游戏归档上传到服务器。浏览器只在当前页面中读取归档，并将需要的文件临时转换成浏览器可以使用的资源。

问题请QQ群:**469524683** 或issus，绝大部分代码为vibe coding产物，低参考价值
...
安卓 Firefox（Gecko）不会把触摸事件直接分发给 `<img usemap>` 热区上的 `<area>`，主菜单的存档/读档/设置等按钮此前只能显示选中状态而无法进入；Web 壳会把落在热区图上的触摸映射回对应功能项，再走游戏原有的点击逻辑，Chromium 与桌面行为不变。

Firefox Android 的 `PointerEvent.movementX/Y` 存在上游缺陷（Bugzilla 1719915，数值异常且不随拖动方向变号），照片贴图页面的贴纸拖动会因此瞬移出画面，该页面在 Firefox Android 上支持性差；桌面 Firefox 与 Chromium 系浏览器不受影响。
