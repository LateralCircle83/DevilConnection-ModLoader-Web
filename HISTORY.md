...
- 增加严格版本匹配的第二层 Remodal 兼容补丁：仅在最终 `index.html` 保留已验证的原版 Remodal 依赖、弹窗和按钮结构时注入独立缩放器，将窄屏媒体查询压缩掉的弹窗恢复为游戏坐标中的 700px 设计宽度，再按 Tyrano 当前 `base_scale` 和移动端可视视口等比居中；未知页面覆盖会中止而不是继承未经验证的布局假设。补丁不覆盖 `$.alert` / `$.confirm`，旋转后有一次 250ms 有界再校准，关闭或退出时完整还原 DOM 和 wrapper 样式。
