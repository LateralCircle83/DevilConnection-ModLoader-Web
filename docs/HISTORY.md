...
- 标题循环修复三个现代浏览器可复现问题：(1) WebKit Bug 156316——本体 `title_main*.mp4` / `title_loop*.mp4` 首帧 PTS 为 1024/12288 ≈ 0.0833s，Safari 拒绝播放不以 0ms 精确开始的 MSE 缓冲导致黑屏；运行时做有界只读解析（moov 视频轨 timescale + 首个 moof 的 tfdt 与首样本 composition offset）得到首帧 PTS，主段 `timestampOffset = -首帧偏移` 把缓冲起点对齐到 0，循环段与每轮追加用 `timestampOffset = bufferedEnd - 首帧偏移` 让首帧精确落在上一段终点，同时消除循环边界的 83ms 间隙。(2) WebKit Bug 157539——对默认应 segments 的带时间戳媒体强制 sequence 时 Safari 只缓冲约半秒，视频 `SourceBuffer` 显式改为 `segments` 模式以片内时间戳串接；MP3 无片段时间戳，仍保持 `sequence` 并用 `appendWindowStart/End + timestampOffset` 无间隙拼接。(3) 所有引擎的 MSE 配额（Chrome ~150MB / Firefox ~100MB / Safari ~290MB）——原游戏循环段反复 append 且从不 `remove()`，缓冲无界增长，停留标题界面最终必然 `QuotaExceededError`；每次循环段追加完成后在同一串行队列里 `remove(0, currentTime-1s)` 回收已播头部，缓冲恒定在“主段 + 循环段 + 1s”量级，播放位置未越过安全边界时跳过。回收复用既有串行队列（新增 `remove` 任务类型，与追加共用 `updateend` 驱动），不改变追加时序、释放与同名实例替换逻辑。
- 临时兜底（WebKitGTK/WPE 环境，待后续正式化或移除）：未配置 `MSE_MAX_BUFFER_SIZE` 时 GStreamer 移植的 SourceBuffer 上限约 15MB，本体主视频 18MB 首次追加即抛 `QuotaExceededError`，上述配额回收在该环境救不回（第一次追加就进不去）。运行时包装 `dc.loadLoopBuffers` 记录真实 VFS 路径（`dc.__dcTitleLoopPaths`），视频轨配额失败时降级为渐进式播放：优先给 `video.src` 赋逻辑路径（如 `./data/video/title_main.mp4`），由 Host kernel 的 `HTMLMediaElement.src` 补丁转成与普通 `[movie]` 相同的受管 URL；路径不可用时退回内存 Blob。复用游戏创建的同一 video 元素，主段播完切循环段并 `loop`，音频走既有 plain-audio 降级；MSE 状态与旧 URL 同步释放，降级 URL 注册在 `dc.__dcTitleLoopFallback`，退出或切换时统一回收并停止降级音频。仅视频轨 `QuotaExceededError` 触发，其他追加/解析错误保持原有失败与释放行为。
...
- Firefox 桌面修复 tap_effect 波纹坐标读取崩溃：游戏自带 `tap_effect/main.js` 在 body 上委托 `mousedown.tap_effect touchstart.tap_effect`，处理器用 `const x = e.clientX || e.targetTouches[0].clientX` 读坐标；Firefox 对键盘激活（Enter/Space）会合成 `clientX=0` 的 mousedown 序列（Chrome 只发 click），`0` 为假值后继续读 `e.targetTouches[0]` 抛 `TypeError: e.targetTouches is undefined`，并中断该次 jQuery 分发中后续处理器。新增严格版本匹配的 `devil-connection-tap-effect` Profile：把两行坐标读取替换为“优先 `clientX`、其次 `targetTouches[0]`、最后回退 0”的防御写法，任何事件类型（含无坐标合成事件、真实触摸）都不再抛错；波纹效果与对话点击时序不变，未知源码或模组覆盖保留原资源并产生可启动警告。
- 修复 iOS/WebKit 多 AudioContext 挂起：游戏在多处各自 `new AudioContext()`——`kag.tmp.audio_context`、waapi 无缝循环 BGM 播放器的模块级 `context`、popopo 插件的 `audioContext`、以及 howler 懒加载的 `Howler.ctx`。iOS/WebKit 下不在用户手势内创建的上下文都以 `suspended` 启动，而宿主“开始游戏”点击不构成 iframe 文档内的用户激活；旧 `unlockAudio` 只在启动时 resume `Howler.ctx`，其余上下文永远无声。新增 Host kernel `tyrano-audio-unlock.js`：包装 `AudioContext`/`webkitAudioContext` 构造器跟踪全部实例（`Reflect.construct` 保留 `instanceof` 语义，创建时顺带尝试一次 resume），并在 iframe 内真实手势（`pointerdown`/`touchend`/`keydown`）统一 resume 已挂起的上下文，同时兜底 `Howler.ctx`、`kag.tmp.audio_context`、`kag.popopo.audioContext`；`pagehide` 时移除手势监听并清空实例表。适配器在引导时与 `startGame` 时各确认一次安装，Chrome/Firefox 下上下文本就 running，resume 为无害空操作。
...
- Host Tyrano 适配新增 image-map 触摸重定向守卫：Gecko（安卓 Firefox）不会把原始触摸事件分发给 `<img usemap>` 背后的 `<area>`（`<area>` 没有布局盒，Gecko 有意不重定向触摸目标，而兼容鼠标事件仍会重定向），主菜单（`menu_tag.png` 贴图的菜单）的存档/读档/设置/返回标题/关闭等 `<area>` 功能项因此只触发 mouseover“选中”而不进入。适配器安装一个 document 级委托 `touchend`，仅当游戏移动端 tap 路径生效（`$.fn.click` 为 `$.fn.tap`）且触摸目标为 `img[usemap]` 时，按 `scWidth`/`scHeight` 设计坐标与 rect/circle/poly 命中测试找到 `<area>`，再调用游戏自身的 `trigger('click').click()` tap 语义；Chromium/WebKit 触摸目标已是 `<area>` 或 PC 原生 click 语义生效时保持完全 inert，不重写 `$.fn.click`/`$.event.tap`、不 `preventDefault`、不改菜单结构与 glink/button 绑定。适配器安装时及模组 hook 完成后的真正启动前都会确认，且幂等不重复挂监听。
